...
public extension Sequence where Element == DateBuilder.ResolvedDate {
    func dates() -> [Date] {
        return map { $0.date() }
    }
...
public extension Sequence where Element == DateBuilder.Day {
    func addingDays(_ days: Int) -> [DateBuilder.Day] {
        return map { $0.addingDays(days) }
    }
    
    func at(_ timeOfDay: @autoclosure () -> TimeOfDay) -> [DateBuilder.ResolvedDate] {
        return map { $0.at(timeOfDay()) }
    }
    
    func at(hour: Int, minute: Int, second: Int = 0) -> [DateBuilder.ResolvedDate] {
...
public extension DateBuilder {
    struct Week {
        ...
        public func addingWeeks(_ weeks: Int) -> Week {
            return Week(base: base, offset: offset + weeks)
        }
...
public extension Sequence where Element == DateBuilder.Week {
    func addingWeeks(_ weeks: Int) -> [DateBuilder.Week] {
        return map { $0.addingWeeks(weeks) }
    }
    
    var firstDay: [DateBuilder.Day] {
        return map(\.firstDay)
    }
    
    var lastDay: [DateBuilder.Day] {
        return map(\.lastDay)
    }
    
    func weekday(_ weekday: DateBuilder.Week.GregorianWeekday) -> [DateBuilder.Day] {
        return map { $0.weekday(weekday) }
    }
    
    var weekendStartDay: [DateBuilder.Day] {
        return map(\.weekendStartDay)
    }
    
    var weekendEndDay: [DateBuilder.Day] {
...
public extension DateBuilder {
    struct Month {
        ...
        public func addingMonths(_ months: Int) -> Month {
            return Month(base: base, offset: offset + months)
        }
...
public extension Sequence where Element == DateBuilder.Month {
    func addingMonths(_ months: Int) -> [DateBuilder.Month] {
    ...
    func weekday(_ ordinal: DateBuilder.Ordinal, _ weekday: DateBuilder.Week.GregorianWeekday) -> [DateBuilder.Day] {
        return compactMap { $0.weekday(ordinal, weekday) }
    }
    
    func first(_ weekday: DateBuilder.Week.GregorianWeekday) -> [DateBuilder.Day] {
...
public extension Sequence where Element == DateBuilder.Year {
    func addingYears(_ years: Int) -> [DateBuilder.Year] {
        return map { $0.addingYears(years) }
    }
    
    var firstMonth: [DateBuilder.Month] {
        return map { $0.firstMonth }
    }
    
    var lastMonth: [DateBuilder.Month] {
