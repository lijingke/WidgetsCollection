...
class ZXPrizeContentLayer: NSObject {
    ...
    func setPrizeImage(_ image: UIImage?, url: String?) {
        zxprizeImage.image = nil
        if let image = image {
            zxprizeImage.image = image
        } else if let url = url {
            zxprizeImage.setUrl(url)
        }
    }

    func setMarkImage(_ image: UIImage?) {
        zxmarkImage.contents = image?.cgImage
        if image != nil {
            zxprizeImage.alpha = 0.35
        }
    }
}
...
class ZXSectorModel {
    ...
    /// 扇形夹角
    static var sectorRadian: CGFloat { return (CGFloat.pi * 2) / CGFloat(sectorCount) }

    static func zxSectorLayers() -> [ZXPrizeContentLayer] {
        var layers: [ZXPrizeContentLayer] = []
