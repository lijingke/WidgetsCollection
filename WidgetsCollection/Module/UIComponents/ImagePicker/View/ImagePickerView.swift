...
class ImagePickerView: UIView {
    ...
    override func layoutSubviews() {
        ...
        let space = width - (numberOfColumns + 1) * margin
        itemWH = space / numberOfColumns
    ...
    lazy var configureView: UIView = {
        ...
        view.backgroundColor = UIColor(displayP3Red: 0 / 255.0, green: 0 / 255.0, blue: 0 / 255.0, alpha: 0.5)
        return view
    }()
}
...
extension ImagePickerView {
    private func pushTZImagePickerController() {
        ...
        imagePickerVC?.photoPickerPageUIConfigBlock = { _, _, _, _, _, doneButton, _, _, _ in
        ...
        // 自定义gif播放方案
        TZImagePickerConfig.sharedInstance()?.gifImagePlayBlock = { _, imageView, gifData, _ in
        ...
        imagePickerVC?.didFinishPickingPhotosHandle = { _, _, _ in
        }

        imagePickerVC?.modalPresentationStyle = .fullScreen
        getViewController().present(imagePickerVC!, animated: true, completion: nil)
    }
    ...
    private func takePhoto() {
        ...
        if authStatus == AVAuthorizationStatus.restricted || authStatus == AVAuthorizationStatus.denied {
            ...
            getViewController().present(alertVC, animated: true, completion: nil)
...
extension ImagePickerView: LxGridViewDataSource {
    ...
    func collectionView(_: UICollectionView, canMoveItemAt indexPath: IndexPath) -> Bool {
    ...
    func collectionView(_: UICollectionView!, itemAt sourceIndexPath: IndexPath!, canMoveTo destinationIndexPath: IndexPath!) -> Bool {
...
extension ImagePickerView {
    @objc private func deleteBtnAction(_ sender: UIButton) {
        if collectionView(collectionView, numberOfItemsInSection: 0) <= selectedPhotos.count {
...
extension ImagePickerView: UIImagePickerControllerDelegate {
    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [UIImagePickerController.InfoKey: Any]) {
        ...
        if type == "public.image" {
            ...
            TZImageManager.default()?.savePhoto(with: image, meta: meta, location: location, completion: { asset, error in
            ...
            if let url = videoURL {
                TZImageManager.default()?.saveVideo(with: url, location: location, completion: { asset, error in
                    ...
                    if error == nil {
                        ...
                        TZImageManager.default()?.getPhotoWith(assetModel?.asset, completion: { photo, _, isDegraded in
...
extension ImagePickerView: TZImagePickerControllerDelegate {
    /// 用户点击了取消
    func tz_imagePickerControllerDidCancel(_: TZImagePickerController!) {
        print("cancel")
    }
    ...
    func imagePickerController(_: TZImagePickerController!, didFinishPickingPhotos photos: [UIImage]!, sourceAssets assets: [Any]!, isSelectOriginalPhoto: Bool) {
        selectedPhotos = photos
        selectedAssets = assets as? [PHAsset] ?? []
        isSelectedOringinalPhoto = isSelectOriginalPhoto
        collectionView.reloadData()

        // 1.打印图片名字
        printAssetsName(selectedAssets)
        // 2.图片位置信息
...
//                }
    ...
    func imagePickerController(_: TZImagePickerController!, didFinishPickingVideo coverImage: UIImage!, sourceAssets asset: PHAsset!) {
    ...
    ///
    /// 如果allowPickingMultipleVideo是YES，将会调用imagePickerController:didFinishPickingPhotos:sourceAssets:isSelectOriginalPhoto:
    func imagePickerController(_: TZImagePickerController!, didFinishPickingGifImage animatedImage: UIImage!, sourceAssets asset: PHAsset!) {
    ...
    /// 决定相册显示与否
    func isAlbumCanSelect(_: String!, result _: PHFetchResult<AnyObject>!) -> Bool {
        //        if albumName == "个人收藏" {
        //            return false
        //        }
        //        if albumName == "Wheel" {
    ...
    /// 决定asset显示与否
    func isAssetCanSelect(_: PHAsset!) -> Bool {
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        if indexPath.item == selectedPhotos.count {
            if chooseConf.showSheet {
                ...
                let takePhotoAction = UIAlertAction(title: takePhotoTitle, style: .default) { _ in
                    self.takePhoto()
                }
                alertVC.addAction(takePhotoAction)
                let imagePickerAction = UIAlertAction(title: "去相册选择", style: .default) { _ in
            ...
            if filename.contains("GIF") && chooseConf.allowPickingGif && !chooseConf.allowPickingMuitlpleVideo {
                ...
                getViewController().present(vc, animated: true, completion: nil)
