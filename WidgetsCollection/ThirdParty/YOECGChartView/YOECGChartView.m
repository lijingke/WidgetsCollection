...
-(instancetype)initWithFrame:(CGRect)frame params: (YOECGParamter*)params{
    if (self = [super initWithFrame:frame]) {
        [self configPar:params];
    }
    return self;
}

-(void)configPar: (YOECGParamter*)params {
    
    self.standard = params == nil ? [YOECGParamter new] : params;
    
    self.negativeNum = self.standard.negativeNum;
    self.positiveNum = self.standard.positiveNum;
...
-(void)drawStaticECGLine:(NSArray *)voltageArr{
    ...
    self.gridView.standard = self.standard;
    self.gridView.secodeLineHeight = 1 * self.standard.oneGridSize;
